...
def _run_hadid(args):
    """Run the Hadid architecton 3D evolution mode."""
    from src.shapes.population import ShapePopulation
    from src.shapes.renderer import render_genome, render_population_grid
    ...
    config = {
        "pop_size": args.pop_size,
        "num_palette_colors": len(HADID_PALETTE),
        "elitism": 2,
        "crossover_rate": 0.7,
        "tournament_k": 3,
    }
    ...
    if args.branch:
        ...
        pop.initialize_architecton()

    print(f"=== Hadid Architecton 3D Evolution ===")
    print(f"Engine: hadid | Population: {args.pop_size}")
    print()
